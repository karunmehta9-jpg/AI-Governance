...
This project contains two related documentation sets, covering two different high-risk systems, deliberately structured to show two different things a conformity pack can look like depending on the system's history.

**Set A — Conformity documentation for an existing high-risk system (Voice Biometric Authentication System, ORG-AI-004):**

| File | Description |
|---|---|
| `technical-documentation.md` | Annex IV–aligned technical documentation, including honestly disclosed gaps |
| `conformity-assessment.md` | A requirement-by-requirement conformity assessment, with an interim conditional governance position pending remediation |
| `human-oversight-and-monitoring-procedure.md` | The operational human oversight and post-market monitoring plan |

**Set B — Conformity documentation for a newly procured replacement system (TalentBridge Screener, ORG-AI-011), built to replace the CV Screening & Ranking System after the governance failures documented in Projects 1 and 2:**

| File | Description |
|---|---|
| `01-intended-purpose.md` | System description and statement of intended purpose |
| `02-risk-management-summary.md` | Risk management approach, with each risk explicitly traced to a lesson learned earlier in this portfolio |
| `03-data-governance.md` | Training data, validation data, and data governance practices |
| `04-human-oversight.md` | Human oversight mechanisms, including a structural fix to the predecessor system's core failure |
| `05-logging-traceability.md` | Logging design and traceability approach |
| `06-performance-monitoring.md` | An extensive post-deployment monitoring plan, including original fairness metrics designed specifically to detect failure modes a single industry-standard metric would miss |

Set A shows what conformity documentation looks like for a system discovered to have gaps. Set B shows what it looks like when a system is built with governance designed in from the start, including explicit, traceable lessons carried forward from this portfolio's own earlier findings — together, the two sets demonstrate the full range of conditions under which an AIGP professional is actually asked to produce this kind of documentation in practice.
